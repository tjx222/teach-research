...
var DWZ = {
	...
	loadLogin:function(){
		if ($.pdialog && DWZ._set.loginTitle) {
			$.pdialog.open(DWZ._set.loginUrl, "login", DWZ._set.loginTitle, {mask:true,width:444,resizable:false,maxable:false,height:174});
		} else {
	...
	init:function(pageFrag, options){
		...
		if (!_doc.isBind(DWZ.eventType.pageClear)) {
			_doc.bind(DWZ.eventType.pageClear, function(event){
				...
				if ($.fn.editor) {
					$("textarea.editor", box).editor(false);
				}
			});
		}
	}
...
(function($){
	if ($.validator) {
		$.validator.addMethod("val_email_user", function(value, element) {
			var emailcheck = /^(\w)+(\.\w+)*@(\w)+((\.\w+)+)$/;
			return this.optional(element) ||!emailcheck.test(value);
		}, "用户名注册不能是邮箱");
...
function initUI(_box){
	...
	if ($.fn.editor) {
		$("textarea.editor", $p).each(function(){
			var $this = $(this);
			var op = {style:1};	
			var eidtorOption = DWZ.jsonEval($this.attr("eidtorOption"));
			$.extend(op,eidtorOption);
			DWZ.debug("eidtorOption: "+DWZ.obj2str(eidtorOption));
			$this.editor(op);
		});
	}
	...
	$("a[target=navTab]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var title = $this.attr("navtitle") || $this.attr("title") || $this.text();
	...
	if ($.fn.dragsort) {
		$(".sortDrag", $p).each(function(){
			var $this = $(this);
			var options = {dragBetween: true};
			
			var dragOptions = DWZ.jsonEval($this.attr("dragOptions"));
			$.extend(options, dragOptions);
			DWZ.debug("dragOptions: "+DWZ.obj2str(dragOptions));
			$this.dragsort(options);
		});
	}
	...
	_reload: function($tab, flag){
		...
		if (flag && url) {
			...
			if ($tab.hasClass("external")){
				...
				var args = $pagerForm.size()>0 ? $($pagerForm[0]).serializeArray() : {}
			...
			if (url) {
				if ($tab.hasClass("external")) {
					...
					if ($.isEmptyObject(op.data)) { //获取pagerForm参数
						var $pagerForm = $("#pagerForm", $panel);
						op.data = $pagerForm.size()>0 ? $($pagerForm[0]).serializeArray() : {}
					}
...
function _getPagerForm($parent, args) {
	...
	if (form) {
		...
		if (args["orderField"]){
			form[DWZ.pageInfo.orderFlag].value = args["orderDirection"] == "desc" ? args["orderField"] +" desc" : args["orderField"]+" asc";
			if(form[DWZ.pageInfo.orderDirection])form[DWZ.pageInfo.orderField].value = args["orderField"];
		}
 ...
 */
function uploadifyError(file, errorCode, errMsg, errString){
	switch(errorCode){
		...
		case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
		case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
		case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:	alertMsg.error("上传失败，错误信息:" + errString);break;
	}
...
(function($){
	...
	$.extend({
		bringBackSuggest: function(args){
			...
			$box.find(":input,img").each(function(){
	...
	$.fn.extend({
		...
		suggest: function(){
			...
			return this.each(function(){
				...
				function _show(event){
					...
					v=$.trim(postData[$input.attr("postField")||"inputValue"]);
					$.ajax({
						global:false,
						...
						success: function(response){
							...
							$.each(response, function(i){
								...
								if(v!="" && liLabel.indexOf(v) <0)return;
								for (var key in this) {
									if (liAttr) liAttr += ',';
									liAttr += key+":'"+this[key]+"'";
								}
		...
		itemDetail: function(ops){
			var op = $.extend({hasFile:false},ops);
			...
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
						...
						uploaderOption:$th.attr("uploaderOption") || "",
				...
				if (op.hasFile){
					...
					var rowNum = 1;
					try{rowNum = ops.files.length;} catch(e){}
					for (var i=0; i<rowNum; i++){
						var $tr = $(trTm);
						var file = ops.files[i];
						$tr.appendTo($tbody).initUI().find("a.btnDel").click(function(){
							$(this).parents("tr:first").remove();
							initSuffix($tbody);
							return false;
						});
						$tr.find("input[name*='title']").val(file.filename);
						$tr.find("input[name*='resId']").val(file.resId);
					}
					initSuffix($tbody);
				}else if(addButTxt) {
...
(function($){
	...
	var Datepicker = function(sDate, opts) {
		...
		//动态minDate、maxDate
		var now = new Date();
		this.opts.minDate = this.opts.minDate != "now" ? now.formatDateTm(this.opts.minDate):"now";
		this.opts.maxDate = this.opts.maxDate != "now" ? now.formatDateTm(this.opts.maxDate):"now";
	...
	$.extend(Datepicker.prototype, {
		...
		getMinDate: function(){
			return this.opts.minDate === 'now' ? new Date() : this._minMaxDate(this.opts.minDate);
		},
		getMaxDate: function(){
		...
		combox:function(){
			...
			return this.each(function(i){
				...
				$("option", $this).each(function(){
					var option = $(this);
					options +="<li><a class=\""+ (value==option[0].value?"selected":"") +"\" href=\"javascript:void(1);\" value=\"" + option[0].value + "\">" + option[0].text + "</a></li>";
				});
				options +="</ul>";
