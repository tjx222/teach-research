	...
	<div class="tabs">
		...
		<div class="tabsContent">
			<div>
																...
																<li><a >${nj.name }</a>
																	<ul>
																		<c:forEach items="${nj.child }" var="jc">
																			<li><a title="${jc.comName }" id="jcTree_${jc.comId }" href="${ctx }/jy/back/jxtx/catalog_tree?comId=${jc.comId }" target="ajax" rel="jiaocai_ml_box">${jc.comName }</a></li>
																		</c:forEach>
																	</ul>
																</li>
															</c:forEach>
				...
				<div class="show"></div>
				<div id="jiaocai_ml_box" class="unitBox" style="margin-left:246px;">
					<div class="prompt">
						<p>
							<span>
								请先在左侧选择-->学段-->学科-->出版社-->年级-->教材，然后再查看信息
							</span>
						</p>
					</div>
				</div>
...
<script type="text/javascript">
	...
	$('.tree li ul li ul li ul li ul li a').click(function (){
		$('.ul_none').hide();
		$('.show').show();
		$('#jiaocai_ml_box').css("margin-left","25px");
		
	});
	$('.show').click(function (){
		$('.ul_none').show();
		$('.show').hide();
		$('#jiaocai_ml_box').css("margin-left","246px");
	});
</script>
