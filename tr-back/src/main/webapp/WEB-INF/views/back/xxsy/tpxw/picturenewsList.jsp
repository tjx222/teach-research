...
<div class="pageContent j-resizeGrid" style="border-left:1px #B8D0D6 solid;border-right:1px #B8D0D6 solid">
	<div style="float: left;" class="panelBar">
		<ul class="toolBar">
			...
			<li><a class="add" href="${ctx }/jy/back/xxsy/tpxw/addPicturenews?orgId=${orgId}" width="780" height="600"  target="dialog" mask="true"><span>发布新闻</span></a></li>
	...
	<form id="pagerForm" action="${pageContext.request.contextPath}/jy/back/xxsy/tpxw/picturenewsList" method="post" onsubmit="return validateCallback(this, reloadtpxw);">
	<table class="table" width="100%" layoutH="110">
		...
		<c:forEach items="${pictureNewsList.datalist}" var="fd">
				...
				<td><input name="ids" value="${fd.id}" type="checkbox"></td>
					...
					<td>
					 ...
					 <td >
						<a title="查看" target="dialog" href="${ctx}/jy/back/xxsy/tpxw/viewPictureNews?id=${fd.id}" value="" class="btnSee" callback="reloadtpxw" mask="true" width="780" height="600"></a>
						<a title="确定要删除吗？" target="ajaxTodo"  href="${pageContext.request.contextPath}/jy/back/xxsy/tpxw/batchdelete?ids=${fd.id}"   value="${fd.id}" class="btnDelete" callback="reloadtpxw"></a>
					 	<c:if test="${fd.isDisplay eq 0}">
							<a title="确认要发布吗?" target="ajaxTodo" href="${ctx }/jy/back/xxsy/tpxw/setView?id=${fd.id}&&flag=1&orgId=${orgId}"  class="btnPunish" callback="notice.release" mask="true" width="780" height="600"></a>
						</c:if>
						<c:if test="${fd.isDisplay ne 0}">
							<a title="要取消发布吗?" target="ajaxTodo" href="${ctx }/jy/back/xxsy/tpxw/setView?id=${fd.id}&&flag=0&orgId=${orgId}"  class="btnPunish2" callback="reloadtpxw" mask="true" width="780" height="600"></a>
						</c:if>
					 </td>
				</tr>
				</c:forEach>
		...
		<div class="panelBar">
			<div class="pages">
					<span>显示</span>
					<select class="combox" name="numPerPage" onchange="navTabPageBreak({numPerPage:this.value},'tpxwindexid')">
			...
			<div class="pagination" rel="tpxwindexid" totalCount="${pictureNewsList.totalCount }" numPerPage="${pictureNewsList.pageSize }" currentPage="${pictureNewsList.currentPage }"></div>
		</div>
		</form> 
</div></div></div>
</body>
<script type="text/javascript" >
	function reloadtpxw(){
		var t = $("#tpxwindexid").find("input[id='title']").val();
		var _title = encodeURI(t);
		var orgId='${orgId}';
		$("#tpxwindexid").loadUrl( _WEB_CONTEXT_+"/jy/back/xxsy/tpxw/picturenewsList",{title:_title,orgId:orgId},function(){
			$("#tpxwindexid").find("[layoutH]").layoutH();
		}		
		);
	}
	$(function(){
