...
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	<h2 class="contentTitle">用户进入课堂时间按天统计</h2>
	<div class="yhjrsj pageContent">
	<div style="width: 430px;">
		...
		<button type="submit" onclick="yhjrsj.loadchart(1)" style="width: 55px;">统计</button>
	</div>
	<div id="yhjrsj_chartHolder" style="width: 1600px; height: 300px"></div>
	<h2 class="contentTitle">用户进入课堂时间实时统计</h2>
	<div style="width: 430px;">
		<label>日期：</label> <input id="startTime2" type="text" name="startTime" 
			style="width: 120px; background-color: #fff; height: 19px;"
			value="<fmt:formatDate value="${model.endTime}" pattern="yyyy-MM-dd"/>" datefmt="yyyy-MM-dd" maxDate="%y-%M-%d"
			readonly="readonly" class="date" />
		<button type="submit" onclick="yhjrsj.loadchart(2)" style="width: 55px;">统计</button>
	</div>
	<div id="yhjrsj_chartHolder2" style="width: 1600px; height: 300px"></div>
</div>
...
var yhjrsj = {
	...
	joinclassByDay:function(result){
		if(result.linesX.length==0){
			//无数据
			$("#yhjrsj_chartHolder").html($("#yh_unit_l").html());
			return false;
		};
		$(".yhjrsj #yh_unit_l").hide();
		$("#yhjrsj_chartHolder").html("");
		...
		var r = Raphael("yhjrsj_chartHolder");
		var lines = r.linechart(
				30, // X start in pixels
				10, // Y start in pixels
				yhjrsj._weight, // Width of chart in pixels
	...
	joinclassTime:function(result){
		if(result.linesX.length==0){
			//无数据
			$("#yhjrsj_chartHolder2").html($("#yh_unit_l").html());
			return false;
		};
		$("#yhjrsj_chartHolder2").html("");
		var axisxstepLength = result.optionsX.length-1;
		var options = {
				axis: "0 0 1 1", // Where to put the labels (trbl)
				axisxstep: axisxstepLength==0?1:axisxstepLength, // x轴间隔数
		...
		var r = Raphael("yhjrsj_chartHolder2");
		var lines = r.linechart(
				30, // X start in pixels
				10, // Y start in pixels
				yhjrsj._weight, // Width of chart in pixels
	...
	loadchart:function(type){
		var url = "${ctx}/jy/back/zbkt/yhjrsjResult";
		if(type==1){
			var startTime = $(".yhjrsj #startTime").val();
			var endTime = $(".yhjrsj #endTime").val();
			...
			var callback = yhjrsj.joinclassByDay;
			yhjrsj.requestAjax(url,data,callback);
		}else{
			var startTime = $(".yhjrsj #startTime2").val();
			startTime = new Date(startTime).Format("yyyy-MM-dd HH:mm:ss");
			var data = {
					'startTime':startTime,
					'flago':type
			}
			var callback = yhjrsj.joinclassTime;
			yhjrsj.requestAjax(url,data,callback);
		}
	},
