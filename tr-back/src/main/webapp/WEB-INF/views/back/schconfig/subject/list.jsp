...
<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="${ctx}jy/back/schconfig/subject/toadd?phaseId=${phaseId}&orgId=${orgId}" target="dialog" mask="true"><span>添加</span></a></li>
			<li><a class="delete" href="${ctx}jy/back/schconfig/subject/delete/{sid_obj}?phaseId=${phaseId}&orgId=${orgId}" target="ajaxTodo" callback="reloadSubjectBox" title="确定要删除吗?"><span>删除</span></a></li>
...
<script type="text/javascript">
	function reloadSubjectBox(){
			$("#org_subjectBox").loadUrl(_WEB_CONTEXT_+"/jy/back/schconfig/subject/list?phase=${phaseId}&orgId=${orgId}",null,function(){
				$("#org_subjectBox").find("[layoutH]").layoutH();
			} );
	}
</script>