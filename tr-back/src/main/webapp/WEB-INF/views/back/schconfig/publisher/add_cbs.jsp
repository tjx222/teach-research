...
<div class="pageHeader" style="border:1px #B8D0D6 solid">
	<div class="searchBar">
		<table class="searchContent">
			<tr>
				<td>
					出版社名称筛选：<input type="text" name="org_cbs_srcname" id="org_cbs_srcname"/>
				</td>
				<td>
					<a class="buttonActive" href="${ctx}jy/back/schconfig/publisher/toadd/custom?phaseId=${phaseId}&subjectId=${subjectId }&orgId=${orgId}" target="dialog"><span>自定义出版社</span></a>
				</td>
			</tr>
...
<div class="pageContent">
	<table style="width:566px;" class="table" layoutH="90">
		...
		<tbody>
			<c:forEach items="${unAddedList }"  var="cbs" varStatus="st">
	...
	<div class="formBar">
		<ul>
			<li><a rel="publisherAddIds" target="selectedTodo" targetType="dialog" href="${ctx }jy/back/schconfig/publisher/save?phaseId=${phaseId}&subjectId=${subjectId }&orgId=${orgId}" class="button" callback="reloadSchCBSAfterAdd"><span>保存</span></a></li>
			<li>
...
function reloadSchCBSAfterAdd(data){
	DWZ.ajaxDone(data);
	...
	$("#org_cbs_srcname",$.pdialog._current).focus(false).click(false).keyup(function(event){
		var $items = $(this);
	...
	function reloadcuntompublisherList(){
		parent.org_add_reloadCbsBox("${phaseId}","${subjectId}");
	}
