...
public class BackUserManageServiceImpl implements BackUserManageService {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(BackUserManageServiceImpl.class);
	...
	public PageList<User> getSysUsers(User user) {
		...
		if (user.getAppId() != null) {
			...
			user.addJoin(QueryObject.JOINTYPE.INNER, "UserSpace a").on(
					"u.id = a.userId and a.roleId = :search_role");
			user.addCustomCondition("", paramMap);
		}
		user.setAppId(null);
		if (user.getName() != null && !"".equals(user.getName())) {
			user.setName(SqlMapping.LIKE_PRFIX + user.getName()
	...
	public User saveUserAccount(Integer userType, Login login, User user) {
		...
		String salt = SecurityCode.getSecurityCode(8, SecurityCodeLevel.Hard,
				false);
		String password = passwordService.encryptPassword(login.getLoginname(),
				"123456", salt);
		...
		// 设置头像和缩略头像
		if (user.getOriginalPhoto() != null
				&& !"".equals(user.getOriginalPhoto())) {
			Resources resources = resourceService.findOne(user
					.getOriginalPhoto());
			if (resources != null) {
		...
		logger.info("add user success,id:{},name:{}", user.getId(),
				user.getName());
		return user;
	}
	 ...
	 */
	@Override
	public Map<String, Object> getAreaUsers(Integer areaId, User user,
			Integer roleId) {
		...
		if (roleId != null) {
			...
			user.addJoin(QueryObject.JOINTYPE.INNER, "UserSpace a").on(
					"u.id = a.userId and a.sysRoleId = :search_role");
		}
		List<Integer> orgIds = new ArrayList<Integer>();
		if (areaId != null) {
			orglist = organizationService.getOrgByAreaId(areaId,
					Organization.UNIT);
		}
	 ...
	 */
	@Override
	public Map<String, Object> findSpaceDataByOrgId(User user) {
		...
		if (StringUtils.isNotEmpty(phaseTypes)) {
			List<MetaRelationship> phases = MetaUtils.getOrgTypeMetaProvider()
					.listAllPhase(org.getSchoolings());
			returnmap.put("phases", phases);
		}
		UserRole umodel = new UserRole();
		umodel.setUserId(user.getId());
		List<UserRole> userRoles = userRoleDao.listAll(umodel);
		List<Role> roles = new ArrayList<>();
		for (UserRole userRole : userRoles) {
			roles.add(roleService.findOne(userRole.getRoleId()));
		}
		// 所有角色
	 ...
	 */
	@Override
	public int saveSchUserSpace(UserSpace us, String[] deptIds) {
		String spacename = "";
		...
		if (us.getId() != null) {
			if (one != null) {
				...
				String newSpaceKey = "r" + usTemp.getRoleId() + "s"
						+ usTemp.getSubjectId() + "g" + usTemp.getGradeId()
						+ "p" + usTemp.getPhaseId();
				String oldSpaceKey = "r" + findOne.getRoleId() + "s"
						+ findOne.getSubjectId() + "g" + findOne.getGradeId()
	...
	public void savePowerScope(Integer userId, Integer roleId, String orgIdsStr) {
		...
		if (!StringUtils.isBlank(orgIdsStr)) {
			...
			for (String orgId : orgIdArray) {
				Organization org = organizationService.findOne(Integer
						.valueOf(orgId));
	...
	public Map<String, Object> findAreaRoleInfo(UserSpace userSpa) {
		...
		if (null != findOneSpa.getDepartmentId()) {
			Organization orgFind = organizationService.findOne(findOneSpa
					.getDepartmentId());
		...
		if (StringUtils.isNotEmpty(conDepIds)) {
			...
			for (int i = 0; i < splitgx.length; i++) {
				if (StringUtils.isNotEmpty(splitgx[i])) {
					gxbmList.add(organizationService.findOne(
							Integer.valueOf(splitgx[i])).getName());
				}
			}
			metaMapInfo.put("gxbmList", StringUtils.join(gxbmList, "、"));
		}
		if (null != findOneSpa.getPhaseId()) {
			MetaRelationship findOnePhase = MetaUtils.getPhaseMetaProvider()
					.getMetaRelationship(findOneSpa.getPhaseId());
			// 通过学段找出对应学科名称
			metaMapInfo.put("subjectMeta", MetaUtils.getPhaseMetaProvider()
					.getMeta(findOneSpa.getSubjectId()));
	...
	public void saveAreaUserRole(UserSpace userSpace, Integer editId) {
		...
		if (null != userSpace.getPhaseId()
				&& userSpace.getPhaseId().intValue() != -1) {
			userSpace.setPhaseType(MetaUtils.getPhaseMetaProvider()
					.getMetaRelationship(userSpace.getPhaseId()).getEid());
		}
	 ...
	 */
	@Override
	public Map<String, Object> detailUserSpace(UserSpace us) {
		Map<String, Object> map = new HashMap<String, Object>();
		...
		if (us != null) {
			...
			MetaRelationship phase = MetaUtils.getPhaseMetaProvider()
					.getMetaRelationship(us.getPhaseId());
			if (phase != null) {
			...
			if(us.getBookId() != null){
				map.put("book", bookService.findOne(us.getBookId()));
			}
			if (us.getDepartmentId() != null && us.getDepartmentId() != 0) {
				Organization org = organizationService.findOne(us
						.getDepartmentId());
	 ...
	 */
	@Override
	public void delUserSpace(UserSpace us) {
	...
	public Map<String, Object> findSpaceDataById(UserSpace us) {
		...
		if (us != null) {
			...
			user.setOrgId(us.getOrgId());
			Map<String, Object> map = findSpaceDataByOrgId(user);
			map.put("us", us);
			Organization org = orgService.findOne(us.getOrgId());
			String areaIds = org.getAreaIds();
			Integer[] areaIdArr = com.tmser.tr.utils.StringUtils
					.toIntegerArray(areaIds.substring(1, areaIds.length() - 1),
							com.tmser.tr.utils.StringUtils.COMMA);
			List<Meta> listAllSubject = MetaUtils.getPhaseSubjectMetaProvider()
					.listAllSubject(us.getOrgId(), us.getPhaseId(), areaIdArr);
			if (us.getSysRoleId().intValue() == SysRole.XKZZ.getId().intValue()) {
				map.put("subjects", listAllSubject);
			} else if (us.getSysRoleId().intValue() == SysRole.NJZZ.getId()
					.intValue()) {
				map.put("grades", MetaUtils.getPhaseGradeMetaProvider()
						.listAllGradeByPhaseId(us.getPhaseId()));
			} else if (us.getSysRoleId().intValue() == SysRole.BKZZ.getId()
					.intValue()) {
				map.put("subjects", listAllSubject);
				map.put("grades", MetaUtils.getPhaseGradeMetaProvider()
						.listAllGradeByPhaseId(us.getPhaseId()));
			} else if (us.getSysRoleId().intValue() == SysRole.TEACHER.getId()
					.intValue()) {
				map.put("subjects", listAllSubject);
				map.put("grades", MetaUtils.getPhaseGradeMetaProvider()
						.listAllGradeByPhaseId(us.getPhaseId()));
				...
				pr.setScope("org");
				map.put("cbslist",
						publisherManageService.listAllPubliserMetaByScope(pr));
				// 学段&学科&年级&出版社---》教材
	 ...
	 */
	@Override
	public void savePowerScopeArea(Integer userId, Integer roleId,
			String areaIdsStr) {
		UserManagescope um = new UserManagescope();
		um.setUserId(userId);
		um.setRoleId(roleId);
	 ...
	 * @param ur
	 * @return
	 * @see com.tmser.tr.back.yhgl.service.BackUserManageService#saveSchUserRole(com.tmser.tr.uc.bo.UserRole)
	 */
	@Override
	public Result saveSchUserRole(UserRole ur) {
		Result rs = new Result();
		if (ur != null) {
			UserRole userRole = new UserRole();
			userRole.setRoleId(ur.getRoleId());
			userRole.setUserId(ur.getUserId());
			UserRole urtemp = userRoleDao.getOne(userRole);
			if (urtemp == null) {
				userRoleDao.insert(userRole);
			} else {
				rs.setCode(Result.FAILED);
				rs.setMsg("用户角色已存在");
			}
		}
		return rs;
