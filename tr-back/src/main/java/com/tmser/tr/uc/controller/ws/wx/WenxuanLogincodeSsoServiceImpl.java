...
public class WenxuanLogincodeSsoServiceImpl implements SsoService {
	private static final Logger logger = LoggerFactory.getLogger(WenxuanLogincodeSsoServiceImpl.class);
	...
	public Login loginFailedCallback(Integer appLocalId,
		...
		if (result != null) {
			...
			if (loginService.findOne(lmodel) == null) {
				...
				if (result.getType() == 300){
					//2003
					role.setSysRoleId(2003);
					role = sysRoleService.findOne(role);
					registerService.saveUser(role.getId(), null, user, login);
					
				} else if (result.getType() == 200) {
					//2002
					role.setSysRoleId(2002);
					role = sysRoleService.findOne(role);
					registerService.saveUser(role.getId(), null, user, login);
				}
				UserManagescope manageScope = new UserManagescope();
				manageScope.setUserId(user.getId());
				manageScope.setRoleId(role.getId());
				manageScope.setOrgId(orgId);
				manageScope.setAreaId(o.getAreaId());
				manageScope.setOrgName(o.getName());
				userManagescopeDao.insert(manageScope);
				return login;
			}else{
				logger.info("{} was existed !",lc);
			}
		}
		return null;
	}
