...
public class MenuServiceImpl extends AbstractService<Menu, Integer> implements MenuService {
	...
	@Override
	public void saveOrUpdateMenu(Menu am) {
		if (null == am.getParentid()) {
			am.setParentid(0);
		}
	 ...
	 * 
	 * @param id
	 * @see com.tmser.tr.back.yygl.service.ApplicationManageService#deleteApplicationManage(java.lang.Integer)
	 */
	@Override
	public void deleteMenuWithChild(Integer id) {
		if (id != null) {
			Menu am = new Menu();
			am.setParentid(id);
			List<Menu> aList = findAll(am);
			if (aList.size() > 0) {
				for (Menu a : aList) {
					deleteMenuWithChild(a.getId());
				}
			}
			delete(id);
		}
