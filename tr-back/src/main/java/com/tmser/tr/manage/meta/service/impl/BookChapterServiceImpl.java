...
public class BookChapterServiceImpl extends AbstractService<BookChapter, String> implements BookChapterService {
	...
	public void saveUpCatalog(BookChapter bc) throws Exception {
		if(StringUtils.isNotEmpty(bc.getChapterId())){//修改章节目录
			...
			Book book = bookDao.get(bc.getComId());
			bc.setChapterId(chapterId);
			bc.setParentEduCode(book.getEduCode());//设置书籍码
			//设置章节编码
			//			String parentId = bc.getParentId();//父类chapterId
			//			Integer orderNum = bc.getOrderNum();//当前的排序值
			//			List<ChapterOrder> ordersChain = jymbservice.getChapterOrderUtils().getOrderChainByChapterIndex(parentId);
			//			ChapterEncodeItems chapterEncodeItems = ChapterEncodeItemsBuilder.buildByParentOrderChain(orderNum, book.getEduCode(), ordersChain);
			//			String chapterCode = jymbservice.getEncodeService().encode(chapterEncodeItems);
