 ...
 *
 * @author wangdawei
 * @version $Id: MyPlanBookServiceImpl.java, v 1.0 2015年3月5日 下午3:28:45 wangdawei Exp $
 */
@Service
@Transactional
public class MyPlanBookServiceImpl extends AbstractService<LessonInfo, Integer> implements MyPlanBookService {
	...
	@Override
	public LessonInfo saveLessonInfo(UserSpace userSpace,String lessonId,String lessonName,Integer planType, Book book,Integer schoolYear,Integer termId){
...
//			resourcesService.delete(lessonPlan.getResId()); //删除存储资源记录
	 ...
	 * @return
	 * @see com.tmser.tr.myplanbook.service.MyPlanBookService#getIsOrNotSubmitLessonPlanByBookId(java.lang.String, java.lang.Integer)
	 */
	@Override
	public Map<String, Object> getIsOrNotSubmitLessonPlanByBookId(
			String bookId, Boolean isSubmit,Integer type) {
	...
	private Integer getCheckResType(Integer planType){
		switch(planType){
		case LessonPlan.JIAO_AN : return ResTypeConstants.JIAOAN;
		case LessonPlan.KE_JIAN : return ResTypeConstants.KEJIAN;
		case LessonPlan.KE_HOU_FAN_SI : return ResTypeConstants.FANSI;
		}
	...
	public Model receiveLessonPlanOfActivity(Integer activityId,Model m) throws FileNotFoundException {
		...
		if(userSpace.getSubjectId().intValue()!=mainUserLessonInfo.getSubjectId().intValue() || userSpace.getGradeId().intValue()!=mainUserLessonInfo.getGradeId().intValue() || schoolYear.intValue()!=mainUserLessonInfo.getSchoolYear().intValue()){
		...
		for(ActivityTracks activityTrack:tracksList){
			...
			//复制教案
			LessonPlan lessonPlan = new LessonPlan(lessonInfo.getId(), activityTrack.getPlanName(), newResId, LessonPlan.JIAO_AN, userSpace.getUserId(), lessonInfo.getSubjectId(), lessonInfo.getGradeId(), lessonInfo.getBookId(), lessonInfo.getBookShortname(), lessonInfo.getLessonId(), activityTrack.getHoursId(), null, lessonInfo.getOrgId(), lessonInfo.getFasciculeId(), schoolYear,termId,userSpace.getPhaseId(),activityTrack.getOrderValue(), new Date(), 1);
	 ...
	 */
	@Override
	public Model receiveLessonPlanOfSchoolActivity(Integer activityId,Model m) throws FileNotFoundException {
		//获取当前用户空间
		UserSpace userSpace = (UserSpace)WebThreadLocalUtils.getSessionAttrbitue(SessionKey.CURRENT_SPACE);
		//学年
		Integer schoolYear = (Integer)WebThreadLocalUtils.getSessionAttrbitue(SessionKey.CURRENT_SCHOOLYEAR);
		...
		List<SchoolActivityTracks> tracksList = schoolActivityTracksService.getActivityTracks_zhengli(activityId);
		if(tracksList==null || tracksList.size()<=0){
			m.addAttribute("result", "fail2");
			m.addAttribute("info", "接受失败，无可接收的教案");
			return m;
		...
		if(temp==null){ //不存在则新增
			...
			lessonInfo.setCrtId(userSpace.getUserId());
			lessonInfo.setCrtDttm(new Date());
			lessonInfo.setCurrentFrom(LessonInfo.FROM_SCHOOLACTIVITY);
			lessonInfo = lessonInfoDao.insert(lessonInfo);
		}else{//已存在则
			lessonInfo = temp;
			lessonInfo.setJiaoanCount(tracksList.size());
		...
		//将主备教案加入到教案表
		for(SchoolActivityTracks activityTrack:tracksList){
			//复制存储资源
			String newResId = resourcesService.copyRes(activityTrack.getResId());
			//复制教案
			LessonPlan lessonPlan = new LessonPlan(lessonInfo.getId(), activityTrack.getPlanName(), newResId, LessonPlan.JIAO_AN, userSpace.getUserId(), lessonInfo.getSubjectId(), lessonInfo.getGradeId(), lessonInfo.getBookId(), lessonInfo.getBookShortname(), lessonInfo.getLessonId(), activityTrack.getHoursId(), null, lessonInfo.getOrgId(), lessonInfo.getFasciculeId(), schoolYear,termId,userSpace.getPhaseId(),activityTrack.getOrderValue(), new Date(), 1);
