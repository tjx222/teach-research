...
public class JyNoticeController extends AbstractController {
  ...
  public Result delteNotices(@RequestParam(value = "noticeIds") List<Long> noticeIds) {
  ...
  public String viewResources(@PathVariable("resType") Integer resType, @PathVariable("resId") Integer resId) {
    String fileKey = "";
    // 如果计划总结，跳转到计划总结查阅页面
    if (resType == ResTypeConstants.JIAOAN) {
  ...
  public ModelAndView getNextNotice(@PathVariable("noticeId") Long noticeId,
    ...
    // 获取下一篇通知
    JyNotice notice = jyNoticeService.getNextNotice(noticeId, receiverState);
    if (notice == null) {
  ...
  public @ResponseBody DeferredResult<Result> listener(
