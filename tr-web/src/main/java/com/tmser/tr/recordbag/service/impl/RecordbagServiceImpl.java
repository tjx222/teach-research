...
public class RecordbagServiceImpl extends AbstractService<Recordbag, Integer> implements RecordbagService {
  ...
  public void initRecordBags() {
    ...
    int id = user.getId();
    String[] sys = new String[] { Recordbag.JXSJ, Recordbag.ZZKJ, Recordbag.JXFS, Recordbag.JYHD, Recordbag.JXWZ,
        Recordbag.JHZJ, Recordbag.TKJL };
    int sort = 1;
  ...
  public void initPlan(LessonPlan plan, Recordbag bag, Model m) {
    ...
    if (bag.getName().equals(Recordbag.JXSJ)) {
      ...
      plan.addOrder(
          "   fascicule_id desc,chapter_level desc,parent_id desc, order_num desc,lesson_id desc,order_value desc ");
