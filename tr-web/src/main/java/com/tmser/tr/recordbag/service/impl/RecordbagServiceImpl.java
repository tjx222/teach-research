...
@Service
@Transactional
public class RecordbagServiceImpl extends AbstractService<Recordbag, Integer>
		implements RecordbagService {
	...
	public void initRecordBags() {
		...
		int id = user.getId();
		String[] sys = new String[] { Recordbag.JXSJ, Recordbag.ZZKJ,
				Recordbag.JXFS, Recordbag.JYHD };
		int sort = 1;
	...
	public Recordbag addBag(String name, String desc) {
		...
		temp.addCustomCondition(
				" and sort=(select max(sort) from teacher_record_bag)", null);
	...
	public void initPlan(LessonPlan plan, Recordbag bag, Model m) {
		...
		if (plan.getPlanName() != null && !plan.getPlanName().equals("")) {// 模糊查询
			...
			plan.addCustomCondition(
					" and planName like '%" + plan.getPlanName() + "%'", null);
			plan.setPlanName("");
		}
		if (bag.getName().equals(Recordbag.JXSJ)) {
			...
			StringBuffer sb = new StringBuffer(" and (planType = "
					+ ResTypeConstants.FANSI + " or planType="
					+ ResTypeConstants.FANSI_OTHER + ")");
	...
	@Override
	public void initActivity(LessonPlan plan, Recordbag bag, Model m, int id,
			List<Record> recordList, int type) {
		Activity act = new Activity();
		if (plan.getPlanName() != null && !plan.getPlanName().equals("")) {// 模糊查询
			m.addAttribute("name", plan.getPlanName());// 将查询条件带过去
			// act.addCustomCondition(" and activityName like
			// '%"+plan.getPlanName()+"%'", null);
			act.setActivityName(SqlMapping.LIKE_PRFIX + ","
					+ plan.getPlanName() + "," + SqlMapping.LIKE_PRFIX);
			// act.setActivityName("");
		}
		...
		PageList<Record> reList = new PageList<Record>(recordList,
				actList.getPage());// 构造Record
	...
	@Override
	public void savePlan(Integer one, Integer id, Integer type, String desc,
			Recordbag bag) {
	...
	@Override
	public void saveActivity(Integer one, Integer id, Integer type,
			String desc, Recordbag bag) {
	...
	public int saveActivity(Record record, int flag, int id) {
		...
		if (at != null) {
			...
			String start = DateUtils.formatDate(at.getStartTime(),
					"MM-dd HH:mm");
			String end = "";
			...
			record.getExt().put("commentsNum",
					String.valueOf(at.getCommentsNum()));
			if (record.getDesc() != null)
				record.setDesc(record.getDesc().trim());
		}
		return flag;
