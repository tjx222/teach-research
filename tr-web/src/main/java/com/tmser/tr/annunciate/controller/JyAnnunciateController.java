...
public class JyAnnunciateController extends AbstractController {
  ...
  public String notForwardIndex(Model m, JyAnnunciateVo vo, Page page, Integer orgID) {
    ...
    vo.addPage(page);
    vo.addOrder("isTop desc,lastupDttm desc");
    vo.addCustomCondition(
        " and orgsJoinIds like '%," + orgId + ",%' and forwardDescription not like '%," + orgId + ",%'",
  ...
  public Result updateAnnunciate(@PathVariable("id") Integer id, JyAnnunciate annunciate) {
    Result result = new Result();
    annunciate.setId(id);
  ...
  public Result updateAnnunciateStatus(@PathVariable("id") Integer id, @RequestParam("status") Integer status) {
    Result result = new Result();
    try {
  ...
  public String getPreviousAnnunciates(Model model, Integer id, Integer status, Integer type) {
    ...
    JyAnnunciate ja = jyAnnunciateService.getPreAnnunciate(id, CurrentUserContext.getCurrentUser().getOrgId(), status,
        type);
    if (ja == null) {
  ...
  public String getNextAnnunciates(Model model, Integer id, Integer status, Integer type) {
    ...
    JyAnnunciate ja = jyAnnunciateService.getNextAnnunciate(id, CurrentUserContext.getCurrentUser().getOrgId(), status,
        type);
    if (ja == null) {
      ja = jyAnnunciateService.findOne(id);
      model.addAttribute("isLast", true);
  ...
  public JyAnnunciate getFirstAnnunciates(Integer id, Integer status, Integer i, Integer type) {
    ...
    if (type == 0) {
      ...
      jyAnnunciate.addCustomCondition("and orgId in (" + CurrentUserContext.getCurrentUser().getOrgId() + ",-1)",
          new HashMap<String, Object>());
    }
