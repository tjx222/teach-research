...
@Service
@Transactional
public class AnnunciatePunishViewServiceImpl extends AbstractService<AnnunciatePunishView, Integer>
    implements AnnunciatePunishViewService {
  ...
  public PageList<JyAnnunciateVo> findPunishsByPage(JyAnnunciateVo vo, Model m) {
    if (SecurityUtils.getSubject().hasRole(SysRole.XZ.name().toLowerCase())
      ...
      vo.setOrgId(CurrentUserContext.getCurrentUser().getOrgId());
      vo.addCustomCondition("and orgId in(" + CurrentUserContext.getCurrentUser().getOrgId() + ",-1)",
  ...
  private void fillViewState(List<JyAnnunciateVo> vos) {
    ...
    List<AnnunciatePunishView> views = annunciatePunishViewDao.findUserView(CurrentUserContext.getCurrentUserId(),
        annunciateIds);
  ...
  public Integer getAnnunciateNum() {
    ...
    jAnnunciate.addCustomCondition("and orgId in (" + CurrentUserContext.getCurrentUser().getOrgId() + ",-1)",
        new HashMap<String, Object>());
  ...
  public List<JyAnnunciate> getNotReadJyList() {
    // TODO Auto-generated method stub
    return jyAnnunciateDao.getNotReadJyList(CurrentUserContext.getCurrentUser().getOrgId(),
