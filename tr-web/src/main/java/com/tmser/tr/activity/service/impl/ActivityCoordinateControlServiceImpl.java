 ...
 *
 * @author wangdawei
 * @version $Id: ActivityCoordinateControlServiceImpl.java, v 1.0 2015年7月15日 下午5:24:12 wangdawei Exp $
 */
@Service
public class ActivityCoordinateControlServiceImpl implements ActivityCoordinateControlService {
	 ...
	 * @param userId
	 * @return
	 * @see com.tmser.tr.activity.service.ActivityCoordinateControlService#getPowerOfZhengli_activity(java.lang.Integer, java.lang.Integer)
	 */
	@Override
	 ...
	 * @param zhengliList
	 * @return
	 * @see com.tmser.tr.activity.service.ActivityCoordinateControlService#mainUserIsZhengliing(java.util.List, java.util.List)
	 */
	@Override
	public <T> boolean mainUserIsZhengliing(List<T> zhubeiList,
			List<T> zhengliList) {
		UserSpace userSpace = (UserSpace)WebThreadLocalUtils.getSessionAttrbitue(SessionKey.CURRENT_SPACE); //用户空间
		...
		for(Object obj:zhubeiList){
			...
			}else if(obj instanceof SchoolActivityTracks){
				element = resZhengliPowerCache.get(((SchoolActivityTracks)obj).getResId());
			}
			if(element!=null){
				Integer userId = (Integer)element.get();
