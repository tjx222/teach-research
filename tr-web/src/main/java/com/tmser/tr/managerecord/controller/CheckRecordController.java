 ...
 *
 * @author tmser
 * @version $Id: CheckRecordController.java, v 1.0 2015年6月4日 上午11:08:01 tmser
 *          Exp $
 */
@Controller
...
public class CheckRecordController extends AbstractController {
  ...
  public String index(Model m, Integer term, Page page, Integer listType) {
    ...
    Map<String, Integer> countNumbers = kejianCheckService.getCheckNumbers(ResTypeConstants.KEJIAN,
        userSpace.getUserId(), term);
    ...
    if (listType == null || listType == 0) {// 查阅课题
      ...
      PageList<CheckInfo> checkInfoList = kejianCheckService.getCheckInfoListByType(ResTypeConstants.KEJIAN,
          userSpace.getUserId(), term, page, false);
      ...
      PageList<CheckInfo> checkInfoList = kejianCheckService.getCheckInfoListByType(ResTypeConstants.KEJIAN,
          userSpace.getUserId(), term, page, true);
  ...
  public String fansiindex(Model m, Integer term, Page page, Integer listType, Integer resType) {
    ...
    Map<String, Integer> khCount = fansiCheckService.getCheckNumbers(ResTypeConstants.FANSI, userSpace.getUserId(),
        term);
    Integer khCheck = khCount.get("checkCount");
    Integer khYijian = khCount.get("yijianCount");
    Map<String, Integer> qtCount = fansiCheckService.getCheckNumbers(ResTypeConstants.FANSI_OTHER,
        userSpace.getUserId(), term);
    ...
    if (listType == null || listType == 0) {// 查阅课题
      ...
      PageList<CheckInfo> checkInfoList = fansiCheckService.getCheckInfoListByType(resType, userSpace.getUserId(), term,
          page, false);
      m.addAttribute("checkInfoList", checkInfoList);
      listType = 0;
    } else {// 查阅意见
  ...
  public String activityindex(Model m, Integer term, Page page, Integer listType) {
    ...
    // 获取当前用户空间
    UserSpace userSpace = (UserSpace) WebThreadLocalUtils.getSessionAttrbitue(SessionKey.CURRENT_SPACE);
    Map<String, Integer> countNumbers = jtbkCheckService.getCheckNumbers(ResTypeConstants.ACTIVITY,
        userSpace.getUserId(), term);
    m.addAttribute("checkCount", countNumbers.get("checkCount"));
    m.addAttribute("yijianCount", countNumbers.get("yijianCount"));
    // 获取查阅数量和意见数量
    if (listType == null || listType == 0) {// 查阅课题
      ...
      PageList<CheckInfo> checkInfoList = jtbkCheckService.getCheckInfoListByType(ResTypeConstants.ACTIVITY,
          userSpace.getUserId(), term, page, false);
      // 处理列表数据
  ...
  private PageList<Activity> getActivityData(PageList<CheckInfo> checkInfoList) {
    ...
    if (!CollectionUtils.isEmpty(idslist)) {
      ...
      model.addCustomCulomn(
          "id,typeId,activityName,typeName,subjectName,gradeName,organizeUserName,startTime,endTime,commentsNum,isShare");
  ...
  public String toCheckLessonPlanInfo(Integer term, Page page, Integer listType, Model m) {
    // 获取当前用户空间
    UserSpace userSpace = (UserSpace) WebThreadLocalUtils.getSessionAttrbitue(SessionKey.CURRENT_SPACE);
    Map<String, Integer> countMap = jiaoancheckService.getCheckNumbers(ResTypeConstants.JIAOAN, userSpace.getUserId(),
        term);
    ...
    if (listType == null || listType == 0) {
      ...
      PageList<CheckInfo> checkInfoPageList = jiaoancheckService.getCheckInfoListByType(ResTypeConstants.JIAOAN,
          userSpace.getUserId(), term, page, false);
  ...
  public ModelAndView getPlainSummary(Integer term, Page page, Integer listType, Integer resType) {
    ...
    Map<String, Integer> khCount = jiaoancheckService.getCheckNumbers(ResTypeConstants.TPPLAIN_SUMMARY_PLIAN,
        userSpace.getUserId(), term);
    Integer khCheck = khCount.get("checkCount");
    Integer khYijian = khCount.get("yijianCount");
    Map<String, Integer> qtCount = jiaoancheckService.getCheckNumbers(ResTypeConstants.TPPLAIN_SUMMARY_SUMMARY,
        userSpace.getUserId(), term);
    Integer qtCheck = qtCount.get("checkCount");
    Integer qtYijian = qtCount.get("yijianCount");
    ...
    if (listType == null || listType == 0) {// 查阅课题
      ...
      PageList<CheckInfo> checkInfoList = jiaoancheckService.getCheckInfoListByType(resType, userSpace.getUserId(),
          term, page, false);
