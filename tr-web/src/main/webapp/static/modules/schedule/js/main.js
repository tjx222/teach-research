define("cal/main", ["require", "jquery", "hogan", "zxui/Lunar", "zxui/Calendar", "zxui/Tip", "./basePopup", "./advancedOption", "./schedule", "common/dateFormat", "./Remoter", "zxui/lib"], 
function(require) {
    function e() {
        v(),
        u(),
    ...
    function v() {
            ...
            process: function(e, t, i) {
                ...
                function(t, r) {
                    ...
                    if (i === a) {
                        ...
                        if ($div = C("<div></div"), $div.attr("class", "ecl-ui-lunar-event"), $div.attr("id", r.id),$div.attr("title", F.eventsMap[r.id].summary), $div.attr("data-etag", r.etag), $div.attr("data-location", r.location), $div.attr("data-description", r.description), $div.attr("data-recurenceId", r.recurrenceid), $div.attr("caluri", r.caluri), r.color && $div.css("background", r.color), $div.html(F.eventsMap[r.id].summary), C(e).append($div[0]), C(".ecl-ui-lunar-event", C(e)).length > 2) {
                            var o = "更多" + (F.dateEvent[i].length - 2) + "个";
                            if (C(e).find(".ecl-ui-lunar-event").last().remove(), 0 === C(".ecl-ui-lunar-more", C(e)).length) $div = C("<div></div"),
                            $div.attr("class", "ecl-ui-lunar-more"),
                            $div.html(o),
                            C(e).append($div[0]);
                            else C(".ecl-ui-lunar-more", C(e)).html(o)
                        }
                    }
                }),
                C.each(a, 
        ...
        function(e) {
            var t = this,
            n = require("zxui/lib").getTarget(e.event);
            if ("A" == n.tagName) return H["new"] = T.compile(V).render({
                text: "新建日程"
            }),
            B["new"] = T.compile(X).render({
                value: e.value,
                week: e.week,
                dataDate: e.value,
                dataTime: y(t.format(e.value, t.dateFormat)).getTime()
            }),
            g({
                type: "new",
                pos: m(e.event),
            ...
            i(),
            !1;
            if (F.eventsMap.hasOwnProperty(C(n).attr("id"))) {
                var r = C(n);
                return H.edit = T.compile(V).render({
                    text: r.html()
                }),
                B.edit = T.compile(Y).render({
                    content: r.attr("data-location"),
                    description: r.attr("data-description"),
                    recurenceId: r.attr("data-recurenceId"),
                    date: e.value + " " + e.week,
                    id: r.attr("id"),
                    caluri: r.attr("caluri"),
                    dataEtag: r.attr("data-etag")
                }),
                g({
                    type: "edit",
                    pos: m(e.event),
            ...
            if (C(n).hasClass("ecl-ui-lunar-more")) {
                for (var r = C(n), a = [], o = F.dateEvent[e.value], s = 0, l = o.length; l > s; s++) a.push(T.compile(W).render({
                    summary: o[s].summary,
                    dataLocation: o[s].location,
                    dataDescription: o[s].description,
                    recurenceId: o[s].recurrenceid,
                    id: o[s].id,
                    caluri: o[s].caluri,
                    dataEtag: o[s].etag,
                    color: o[s].color && "background:" + o[s].color
                }));
                H.more = T.compile(V).render({
                    text: e.value + "(" + e.week + ")"
                }),
                B.more = a.join(""),
                g({
                    type: "more",
                    pos: {
                        x: r.parent().offset().left,
                        y: r.parent().offset().top
                    },
                    arrow: !1,
                    isRemoveOtherTips: !0,
                    triggers: "ecl-ui-lunar-more",
                    addClassName: "ecl-ui-tip-more",
                    e: e.event,
                    target: n
                })
            }
        ...
        function() {
            var e = this,
            t = T.compile(G).render({
                dataTime: w,
                startTime: y(w).getTime()
            });
            M.show({
                mask: !0,
                title: "新建日程",
            ...
            e.format(new Date, e.dateFormat)),
            new k({
                monthes: 1,
                prefix: "ecl-ui-cal",
                dateFormat: "yyyy-MM-dd",
                triggers: C("#add-cal-date-target")[0],
                target: C("#add-cal-date-target")[0],
                value: w,
                onPick: function() {
                    C("#ecl-ui-tip-body-advance").attr("data-date", this.target.value),
                    C("#ecl-ui-tip-body-save").attr("date-time", y(this.target.value).getTime())
                }
            }).render(),
            C("#add-cal-event-popup").focus(),
            C("#add-cal-date-target").on("change", 
            function() {
                var e = C(this).val();
                if (!O.isDateFormat(e)) C(this).val(C("#ecl-ui-tip-body-advance").attr("data-date"));
                else C("#ecl-ui-tip-body-advance").attr("data-date", e),
                C("#ecl-ui-tip-body-save").attr("date-time", y(e).getTime())
            })
        });
        x.on("navigate", 
        function() {
    ...
    j = 0,
    q = 0,
    W = '<div class="ecl-ui-tip-body-row ecl-ui-tip-body-row-more" title="{{summary}}" style="{{color}}" data-etag="{{dataEtag}}" id="{{id}}" caluri="{{caluri}}" data-recurenceId="{{recurenceId}}" data-location="{{dataLocation}}" data-description="{{dataDescription}}">{{{summary}}}</div>',
    ...
    X = '<div class="ecl-ui-tip-body-row">时间：{{value}}&nbsp;&nbsp;{{week}}</div><div class="ecl-ui-tip-body-row">日程：<input type="text" id="add-cal-popup" class="ecl-ui-tip-body-event"/></div><div class="ecl-ui-tip-body-row"><span class="ecl-ui-tip-body-save" date-time="{{dataTime}}">保存</span><span class="ecl-ui-tip-body-advance" data-date= "{{dataDate}}">高级选项&nbsp;&gt;</span><span class="ecl-ui-tip-body-error">请填写内容</span></div>',
    G = '<div class="ecl-ui-tip-body-row">时间：<input type="text" value="{{dataTime}}" class="ecl-ui-tip-body-time" id="add-cal-date-target" /></div><div class="ecl-ui-tip-body-row">日程：<input type="text" maxlength="50" id="add-cal-event-popup" class="ecl-ui-tip-body-event"/></div><div class="ecl-ui-tip-body-row"><span id="ecl-ui-tip-body-save" class="ecl-ui-tip-body-save" date-time="{{startTime}}">保存</span><span id="ecl-ui-tip-body-advance" class="ecl-ui-tip-body-advance" data-date= "{{dataTime}}">高级选项&nbsp;&gt;</span><span class="ecl-ui-tip-body-error">请填写内容</span></div>',
    U = {
        0: "周日",
        1: "周一",
        2: "周二",
