define(["require","zepto","iscroll"], function (require) {
	...
	function init() {
		...
		$('.head').click(function (){
			...
			$("#discuss_iframe").attr("src",_WEB_CONTEXT_+"/jy/comment/discussIndex?activityId="+activityId+"&crtId="+userId+"&canReply="+canReply+"&typeId=5");
		});
		...
		$('.content_list figure').eq(4).click(function (){
			...
			if($("#commentBox").attr("src")==""){
				...
				var url = _WEB_CONTEXT_+"/jy/comment/list?resType=5&resId="+resId+"&title="+encodeURI(title)+"&authorId="+authorId+"&vn=/activity/view/tbComment";
				if($(this).attr("isOver")=="true"){
		...
		$('.content_list_sp figure').eq(0).click(function (){
			closeAll(); 
			$('.act_info_wrap').css({"width":"100%","height":"100%","position":"fixed","z-index":"102"});
			$('.act_info_wrap').show(); 
			$('.mask').show();  
		});
		...
		$('.cy_list1').click(function (){
			closeAll();
			var activityId = $.trim($(this).children('input').val());
			$("#discuss_iframe").attr("src",_WEB_CONTEXT_+"/jy/comment/discussIndex?activityId="+activityId+"&canReply=true&typeId=5&"+ Math.random());
		...
		$('.ck_list1').click(function (){
			...
			if($("#commentBox").attr("src")==""){
				...
				var authorId = $(this).attr("authorId");
				if($(this).attr("isOver")=="true"){
					$("#commentBox").attr("src",_WEB_CONTEXT_+"/jy/comment/list?flags=1&resType=5&resId="+resId+"&title="+encodeURI(title)+"&authorId="+authorId);
				}else{
					$("#commentBox").attr("src",_WEB_CONTEXT_+"/jy/comment/list?flags=0&resType=5&resId="+resId+"&title="+encodeURI(title)+"&authorId="+authorId);
				}
			}
	...
	function bindEvent(){
		...
		$("#overSpjy").click(function(){  //绑定结束主题研讨按钮
			overActivitySpjy($(this).attr("activityId"));
		});
	...
	window.showLessonPlanTrack_canyu = function(planId,activityId){
		$("#iframe1").attr("src",_WEB_CONTEXT_+"/jy/activity/showLessonPlanTracks?editType=0&planId="+planId+"&activityId="+activityId);
	};
	...
	//结束活动
	function overActivitySpjy(activityId){
		if(window.confirm("您确定要结束吗？结束后，所有人将不能参与此集备活动！")){
			$.ajax({  
		        async : false,  
		        cache:true,  
		        ...
		        success:function(data){
		        	if(data.result=="success"){
		        		...
		        		//window.location.href = _WEB_CONTEXT_+"/jy/activity/viewSpjyActivity?site_preference=mobile&id="+activityId;
		        	}else{
		        		successAlert("系统出错");
		        	}
		        }  
	...
	window.addmoredatas = function(data){
		...
		$("#partake_discussion_content1").append(content.find(".partake_discussion"));
		 var partake_discussion = new IScroll('#partake_discussion',{
     		 scrollbars:true,	
     		 mouseWheel:true,
     		 fadeScrollbars:true,
