define(["require","zepto","iscroll"], function (require) {
	...
	$(function(){
	 ...
	 window.checkreply = function(obj){
		...
		var htmlStr = "";
		if(classname=="reply"){
			htmlStr = '<div id="'+id+'" class="reply_comment_div" ><textarea placeholder="回复'+uname+'" class="contentval" rows="4" cols="72" autofocus style="width:99%;height:6rem;margin-top:1rem;border:0.083rem #bdbdbd solid;" ></textarea>'+
			'<div onclick="submitReply('+parentid+')" class="btn_fs">发送</div></div>';
		}else if(classname=="reply1"){
			htmlStr = '<div id="'+id+'" class="reply_comment_div" ><textarea placeholder="回复'+uname+'" class="contentval" rows="4" cols="62" autofocus style="width:99%;height:6rem;margin-top:1rem;border:0.083rem #bdbdbd solid;" ></textarea>'+
			 '<div onclick="submitReply('+parentid+');" class="btn_fs">发送</div></div>';
		}
	    $(obj).after(htmlStr);
	    $(".contentval")[0].focus(); 
