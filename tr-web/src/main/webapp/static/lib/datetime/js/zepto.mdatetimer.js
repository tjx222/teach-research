...
(function ($) {
	...
	var F = {
			...
			formatDate : function(year, month, day, hour, minute, second,option){
			...
			getDateValue : function(value,option){
				if(option.format==2){
    ...
    $.Mdatetimer = function(elm, options) {
		this.$elm = $(elm);
		this.opts = options;
		return this.init();
	};
	...
	$.extend($.fn,{
		mdatetimer:function(options) {
			var mdatetimers = [];
			this.each(function() {
				mdatetimers.push(new $.Mdatetimer(this, $.extend({}, $.Mdatetimer.defaults, options)));
			});
		return $(mdatetimers); 
	   }
	});
	...
	$.extend($.Mdatetimer,{
	  ...
	  prototype : {
	  	init : function(){
				...
				var defaultDate = this.$elm.val()!='' ? F.getDateValue(this.$elm.val(),_this.opts) : new Date(),
				...
				if(this.opts.mode != 1){
					var hourStr = '';
					var hourcount = this.opts.mode == 3 ? 12 : 24;
					...
					if(this.opts.mode==2 || this.opts.mode==3){
						$('.mt_second .mt_note').html('&nbsp;');
					}
				}

				//初始化点击this.$elm事件
				this.$elm.on('tap', function(){
					curerntElm = $(this);
				...
				$('.mt_ok', container).on('tap', function(){
					...
					var minute = mpMinute.find('.selected').data('minute');
					var second = mpSecond.find('.selected').data('second');
					curerntElm.val(F.formatDate(year, month, day, hour, minute, second,_this.opts));
					_this.hidePanel();
					_this.opts.onOk && typeof _this.opts.onOk=='function' && _this.opts.onOk(container);
				});
				$('.mt_cancel', container).on('tap', function(){
					_this.hidePanel();
					_this.opts.onCancel && typeof _this.opts.onCancel=='function' && _this.opts.onCancel(container);
				});
				$('.mt_setnow', container).on('tap', function(){
					var n = new Date();
					curerntElm.val(F.formatDate(n.getFullYear(), n.getMonth(), n.getDate(), n.getHours(), n.getMinutes(), n.getSeconds(),_this.opts));
					_this.hidePanel();
				});
		...
		renderHTML : function(){
				var stime = this.opts.timeStart + ':00';
				var etime = this.opts.timeStart + this.opts.timeNum + ':00';
