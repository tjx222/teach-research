			...
			if (defaults.type == 'csv' || defaults.type == 'txt') {
				...
				$(el).find('thead').find('tr').each(function() {
					...
					$(this).filter(':visible').find('th').each(function(index, data) {
						if ($(this).css('display') != 'none') {
							if (defaults.ignoreColumn.indexOf(index) == -1) {
								if($(data).attr('rowspan')>0 && $(data).attr('colspan')>0){
									excel += "<th rowspan='"+$(data).attr('rowspan')+"' colspan='"+$(data).attr('colspan')+"'>" + parseString($(this)) + "</th>";
								}else if($(data).attr('rowspan')>0){
									excel += "<th rowspan='"+$(data).attr('rowspan')+"'>" + parseString($(this)) + "</th>";
								}else if($(data).attr('colspan')>0){
									excel += "<th colspan='"+$(data).attr('colspan')+"'>" + parseString($(this)) + "</th>";
								}else{
