...
<c:if test="${jfn:isNormal()}">
	...
	%>
		...
		<li style="background:none;">
			   <span>转到:<input type="text" class="wid go_page">页</span>
		</li>
		<li id="go" class="goto_page"><strong onclick="javascript:<%=isAjax?"gotoPageAjax":"gotoPage"%>(this,'<%=url%>',<%=data.getTotalPages()%><%=isAjax?","+callback+(isJsonp?",true":",false"):""%>)">GO</strong></li>
		</ol>
	</div>
	<%}%>
	</c:when>
	...
	<script>
		$(".go_page").keypress(function(e){
			...
			$(form).find("li.goto_page strong").trigger("click");
	    	return false;
    		}
		});
</script>
...
<c:if test="${jfn:isMobile() || jfn:isTablet()}">
	...
	<%
	if(data.getTotalPages() > 1){
			dataType = dataType == null || "".equals(dataType.trim()) ? "json":dataType;
		    int  currentpage = data.getCurrentPage();//获取当前页码
		    int totalPage = data.getTotalPages(); //获取总页数
	%>
	<div id="loadPage" style="width: 80%; height: 3rem;line-height:3rem;text-align:center;font-size:1.333rem; margin: 1.5rem auto;cursor:pointer;">
	<%if (totalPage > 1) { %>
		<div nextPage="<%=currentpage+1 %>" totalPage="<%=totalPage %>" onclick="javascript:turnPageAjax_m(this,'<%=url%>',<%=callback%>,'<%=dataType%>')">点击加载更多</div>
	<%}else if(totalPage<=1){%>'
		<div>没有更多</div>
	<%}%>
	</div>
