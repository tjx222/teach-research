...
<%@ attribute name="showTip" type="java.lang.Boolean" required="false" description="是否限制提示" %>
...
<c:if test="${empty showTip}">
    <c:set var="showTip" value="false"/>
</c:if>
...
<!-- production -->
<input type="text" placeholder="请选择文件" value="请选择文件" class="file txt_txt xj_sc" readonly="readonly" name="${originFileName }" id="${originFileName }"/>
<span id="${originFileName }_file_process" class="mes_file_process">
</span>
...
var uploader_${name} = new plupload.Uploader({
	...
	init: {
		PostInit: function() {
			var inithtml="";
			<c:if test="${showTip }">
			inithtml = '文件大小不能超过${fileSize}M<br/>	允许文件类型：${fileType}';
			</c:if>
			document.getElementById('${originFileName }').value = '请选择文件';
			document.getElementById('${originFileName }_file_process').innerHTML = inithtml;
