...
<div id="wrapper">
	...
	<section>
	    <form id="hiddenForm" action="${ctx }jy/managerecord/check/2-3?_HS_=${empty param._HS_ ? 2 :param._HS_+1 }" method="post">
		...
		<div class="managerecord_bottom_wrap">
			<div class="managerecord_check_top" style="width: 90%;height:6rem;margin: 0 auto;border-bottom:0.083rem solid #D0D1D2; ">
				<div class="managerecord_slide">
					<span class="${resType==2?'mana_act':''}" data-resType="2">课后(${khCount })</span><span class="${resType==3?'mana_act':''}" data-resType="3">其他(${qtCount })</span>
				</div>
			...
			<div class="referCourseware_cont_box">
				<div class="referCourseware_cont_box1">
				    <c:forEach items="${checkInfoList.datalist }" var="checkInfo">
						<div class="courseware_ppt viewFanSi"  data-id="${checkInfo.resId}" data-resType="${checkInfo.resType }">
							<div class="courseware_img_fs">反思</div>
				...
				<c:if test="${empty checkInfoList.datalist }"><div class="content_k"><dl><dd></dd><dt>您还没有可查阅的反思，请稍后再来吧！</dt></dl></div></c:if>
			</div>
				   ...
				   <c:forEach items="${checkMapList}" var="checkMap">
						<div class="referCourseware_content referCourseware_content_two">
							<p class="referCourseware_content_title">
								<span>课题名称：</span>
								<b data-resId="${checkMap.checkInfo.resId}" data-type="2" data-resType="${checkMap.checkInfo.resType }">${checkMap.checkInfo.title }</b>
							</p>
