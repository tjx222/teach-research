			...
			<div class="check_tch_wrap">
				...
				<div class="nav_tab">
					<c:if test="${type!=0&&type!=1}">
					    <ul id="selectType">
							<li><a href="jy/check/lesson/2/tch/${u.id}?grade=${grade}&subject=${subject}&fasciculeId=${fasciculeId}&_HS_=${empty param._HS_ ? 2 :param._HS_+1 }">课后反思</a></li>
							<li><a class="header_act">其他反思</a></li>
					    </ul>
			        </c:if>
		         </div>
			</div>
			<div class="content_bottom" id="check_c_b">
				<div id="scroller">
						...
						<c:when test="${!empty resList }">
							<c:forEach items="${resList }" var="res" varStatus="st">
								<div class="courseware_ppt">
									...
									<a href="jy/check/lesson/3/tch/other/${userId}/view?fasciculeId=${fasciculeId }&planId=${res.planId}&zx=${param.wc }&tj=${param.sc }&cy=${param.cc }"><h3><ui:sout value="${res.planName }" length="16" needEllipsis="true" /></h3>
					...
					<c:if test="${empty resList }">
					    <!-- 无文件 -->
						<c:if test="${type==0}">
						       <div class="content_k"><dl><dd></dd><dt>该教师还未提交“教案”，您可以提醒他提交，或者查阅其他教师的工作 !</dt></dl></div>
						</c:if>
						<c:if test="${type==1}">
						        <div class="content_k"><dl><dd></dd><dt>该教师还未提交“课件”，您可以提醒他提交，或者查阅其他教师的工作 !</dt></dl></div>
						</c:if>
						<c:if test="${type==2}">
						        <div class="content_k"><dl><dd></dd><dt>该教师还未提交“课后反思”，您可以提醒他提交，或者查阅其他教师的工作 !</dt></dl></div>
						</c:if>
					</c:if>
