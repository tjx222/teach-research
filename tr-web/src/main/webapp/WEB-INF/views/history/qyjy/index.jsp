...
<head>
	<meta charset="UTF-8">
	<ui:htmlHeader title="历年资源-校级教研"></ui:htmlHeader>
				...
				<table class="resources_table">
					...
					<c:forEach items="${result.data.datalist }" var="activity">
						<tr>
							...
							<td class="partake" onmousemove="showOrgList('${activity.id}');">${activity.orgsJoinCount }
	...
	<c:if test="${result.role=='notCreate' || (result.role=='canCreate' && result.sv.typeId==2)}">
		...
		<div class="pages"> 
			...
			<form id="pageForm" name="pageForm" method="post">
				<ui:page url="${ctx }jy/history/${result.sv.schoolYear}/qyjy/index" data="${result.data}"/>
