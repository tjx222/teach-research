...
<head>
	<meta charset="UTF-8">
	<ui:htmlHeader title="">我的教案</ui:htmlHeader>
...
<form id="pageForm"  name="pageForm" method="post" action="${ctx}jy/history/${searchVo.schoolYear }/zxja/index" onsubmit="dosearch();">
	...
	<c:if test="${!empty jiaoanList.datalist}">
	<div class="resources_table_wrap">
		<table class="resources_table">
			...
			<c:forEach var="plan" items="${jiaoanList.datalist }">
				<tr>
					<td style="text-align:left;">
						...
						<c:if test="${plan.isScan }"><span class="yue" infoId="${plan.infoId }" planType="0"></span></c:if>
						<c:if test="${plan.isComment }"><span class="ping" planId="${plan.planId }" planType="0"></span></c:if>
					</td>
		...
		<div class="resources_table_bottom">
			...
			<ui:page url="${ctx}jy/history/${searchVo.schoolYear }/zxja/index" data="${jiaoanList}"  />
