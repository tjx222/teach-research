...
<body> 
	...
	<div class="jitibeike_content">
			...
			<c:forEach var="activityMap" items="${activityMapList }">
				<div class="jitibeike_con_type">
					<div class="jitibeike_con_type_title">
						<a href="${ctx}jy/teachingView/view/chayueActivity?activityId=${activityMap['activity'].id}&typeId=${activityMap['activity'].typeId}" target="_blank">
							<span <c:if test="${activityMap['activity'].typeId==2 }">style="background:#FD8484"</c:if><c:if test="${activityMap['activity'].typeId==3 }">style="background:#FD8484"</c:if>>${activityMap['activity'].typeName }</span>
		...
		<form id="pageForm"  name="pageForm" method="post">
			<ui:page url="${ctx}jy/teachingView/teacher/list_activity" data="${activityList}"  />
