...
<body> 
	...
	<div class="jyyl_nav">
		...
		<jy:nav id="jyyl_m_cylb">
			...
			<jy:param name="name" value="查阅听课记录"></jy:param>
			<jy:param name="urlxqlb" value="jy/teachingView/manager/m_check_lecture?userId=${user2.id}&termId=${searchVo.termId}&orgId=${searchVo.orgId}&phaseId=${searchVo.phaseId}"></jy:param>
		</jy:nav>
	</div>
		...
		<div class="teachingTesearch_jitibeike_con">
			<div class="teachingTesearch_jitibeike_outBox">
					<div class="teachingTesearch_jitibeike_outBox_type show">
						<c:if test="${not empty lectureList.datalist}">
						<table cellpadding="0" cellspacing="0" class="teachingTesearch_jitibeike_table">
							<tr>
								...
								<th style="width:10%;">查阅状态</th>
							</tr>
								<c:forEach var="kt" items="${lectureList.datalist}">
									  <tr id="${kt.id}">
									     <td class="jitibeike_td1">
									     	<a href="${ctx}jy/teachingView/view/LectureRecord?id=${kt.id}&showType=manager" target="_blank">
											     <b>${kt.type==0?'【校内】':'【校外】'}</b>
									    ...
									    <td class="jitibeike_td7">
										    <c:if test="${not empty chekInfoLectureData[kt.id] }">
										    	已查阅
											</c:if>
										    <c:if test="${empty chekInfoLectureData[kt.id] }">
										    		未查阅
											</c:if>
									    </td>
									  </tr>
						...
						<c:if test="${empty lectureList.datalist}">
							<!-- 无文件 -->
			   				<div class="nofile">
								<div class="nofile1">
									暂时还没有数据，稍后再来查阅吧！
				...
				<form  name="pageForm" method="post">
					<ui:page url="${ctx}jy/teachingView/manager/m_check_lecture?userId=${searchVo.userId}&termId=${searchVo.termId}&phaseId=${searchVo.phaseId}&orgId=${searchVo.orgId}" data="${lectureList}"  />
