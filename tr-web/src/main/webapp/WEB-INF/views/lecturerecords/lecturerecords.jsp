...
<div class="jyyl_nav">
		...
		<c:choose>
				<c:when test="${pl=='1'}">
					<jy:nav id="bkbtkju">
						<jy:param name="name">我的备课本</jy:param>
						<jy:param name="name2">查看听课记录</jy:param>
					</jy:nav>
				</c:when>
				<c:otherwise><jy:nav id="ck"></jy:nav></c:otherwise>
			</c:choose>
	</div>
<div class="check_teacher_wrap">
	<div class="check_teacher_wrap2"> 
			...
			<c:if test="${lr.type==1}">
			  	<div class="record_sheet_cont" style="height:auto;">
					<div class="r_s_c">
						...
						<b>听课地点</b>
						<strong style="width:195px;">${lr.lectureAddress}</strong>
					</div>
					<div class="r_s_c">
		...
		<c:if test="${pl=='1'}"><!--备课本查看过来有回复列表-->
			<iframe onload="setCwinHeight(this,false,200)" src="jy/lecturereply/reply?resId=${lr.id}&authorId=${lr.lecturepeopleId}&teacherId=${lr.teachingpeopleId}" id="commentBox" width="100%" style="border:none;"scrolling="no"></iframe>
		</c:if>
	</div>
</div>
