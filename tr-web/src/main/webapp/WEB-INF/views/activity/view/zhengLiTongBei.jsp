...
<div class="act_modify_wrap">
	<div class="act_modify">
		...
		<div class="act_modify_content">
			<iframe id="iframe3" frameborder="0" scrolling="no" width=100% height=100% src="jy/activity/getZhengliTrackList?site_preference=mobile&activityId=${activity.id}"></iframe>
		</div>
		<div class="act_modify_btn_1">
			...
			<%-- <c:if test="${activity.organizeUserId == _CURRENT_USER_.id && activity.spaceId == _CURRENT_SPACE_.id && !activity.isSend}">
				<c:if test="${mainUserZhengliing }">
					<input type="button" value="发给参与人"  class="input1" title="主备人正在整理，请稍后再发送"/>
				</c:if>
				<c:if test="${!mainUserZhengliing }"> --%>
					<input type="button" value="发给参与人"  class="input" activityId="${activity.id}" id="fasong"/>
				<%-- </c:if> --%>
			</c:if>
			<c:if test="${activity.mainUserId == _CURRENT_USER_.id && activity.isSend}">
			<%-- <c:if test="${activity.organizeUserId == _CURRENT_USER_.id && activity.spaceId == _CURRENT_SPACE_.id && activity.isSend}"> --%>
	...
	<div class="act_participants">
		...
		<div class="act_participants_content">
			...
			<c:if test="${zhubeiList!=null && fn:length(zhubeiList)>0 }">
				<iframe id="iframe2" style="width:33rem;margin:0 auto;display: block;height:100%;" frameborder="0" scrolling="no" src="${pageContext.request.contextPath }/jy/activity/getYijianTrackList?site_preference=mobile&planId=${zhubeiList[0].id }&activityId=${activity.id}"></iframe>
			</c:if> 
		</div>
	</div>
</div>
<div class="partake_discussion_vrap">
	<div class="partake_discussion1" >
		...
		<div class="partake_discussion_content" >
			...
			<div class="clear"></div>
			<div  class="partake_discussion_content1" > 
				<!-- 加载讨论内容 --> 
...
<div class="view_comments_wrap">
	<div class="view_comments">
		...
		<iframe id="commentBox" style="width:100%;height: 40rem;border: none;" ></iframe> 
	</div>
</div>
	...
	<section>
		<div class="content">
			<div class="content_bottom1">
				...
				<div class="content_bottom1_left" id="zhubei_zhengli">
					<h3></h3>
					<ul>
						<c:forEach var="zhubei" items="${zhubeiList }" varStatus="status">
				...
				<div class="content_bottom1_center" style=" z-index: 1001;">
					<c:if test="${zhubeiList!=null && fn:length(zhubeiList)>0 }">
					<iframe id="iframe1" editType="1" style="width:100%;height:100%;" name="iframe1" frameborder="0" scrolling="no" src="${pageContext.request.contextPath }/jy/activity/showLessonPlanTracks?planId=${zhubeiList[0].id }&activityId=${activity.id}&editType=1"></iframe>
					</c:if>
				</div>
