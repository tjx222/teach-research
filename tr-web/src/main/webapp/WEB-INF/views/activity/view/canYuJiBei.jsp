...
<head>
<ui:htmlHeader title="参与同备教案"></ui:htmlHeader>
...
//结束活动
function overActivity(activityId,obj){
...
//保存修改
function saveEdit(){
	window.frames["iframe1"].saveLessonPlanTracks(0);
}
//显示主备教案的修改教案
...
function scanLessonPlanTrack(resId,orgId){
	//window.open(_WEB_CONTEXT_+"/jy/scanRes?resId="+resId+"&orgId="+orgId,"hidenframe");
...
<div class='partake_info_cont'>
	...
	<div class='zb_lessonplan clearfix'>
		...
		<div class='zb_lessonplan_right'>
			...
			<h4 class='zb_lessonplan_right_h4'><input type="checkbox" onclick="wantToEdit(this);" id="checkbox1"/><label htmlFor='edit_lessonplan'>修改教案</label></h4>
			...
			<div class='zb_lessonplan_cont'>
				...
				<div style="width:0px;height: 0px; display: none;" id="kongdiv"></div>
					<c:if test="${zhubeiList!=null && fn:length(zhubeiList)>0 }">
						<iframe width="100%" height="495px" id="iframe1" name="iframe1" style="border:none;border-radius: 6px;" frameborder="0" scrolling="no" src="${pageContext.request.contextPath }/jy/activity/showLessonPlanTracks?planId=${zhubeiList[0].id }&activityId=${activity.id}&editType=0"></iframe>
					</c:if>
				</div>
			</div>
		</div>
	...
	<div class='edit_zb_lessonplan clearfix'>
		...
		<div class='edit_zb_lessonplan_right'>
			<div class='edit_zb_lessonplan_title'>
				...
				<h5 class='edit_zb_lessonplan_right_h5' onclick="receiveLessonPlan('${activity.id}');"><span></span>接收教案</h5>
				</c:if>
			</div>
