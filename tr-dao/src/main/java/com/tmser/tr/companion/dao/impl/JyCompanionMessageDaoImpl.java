 ...
 */
@Repository
public class JyCompanionMessageDaoImpl extends AbstractDAO<JyCompanionMessage, Integer> implements JyCompanionMessageDao {

	@Override
	public List<JyCompanionMessage> findMessage(Integer userId, Integer userIdCompanion, Date startDate, Integer pageSize, Integer pageNum) {
	...
	public PageList<JyCompanionMessage> findMessageWithPage(JyCompanionMessage message) {
		...
		if (message.getSenderTime() != null) {
			sql = "select * from JyCompanionMessage where senderTime>=:senderTime and " + "(userIdSender=:userIdSender and userIdReceiver=:userIdReceiver ) or "
					+ "(userIdSender=:userIdReceiver and userIdReceiver=:userIdSender) order by senderTime desc";
		} else {
			sql = "select * from JyCompanionMessage where " + "(userIdSender=:userIdSender and userIdReceiver=:userIdReceiver ) or "
					+ "(userIdSender=:userIdReceiver and userIdReceiver=:userIdSender) order by senderTime desc";
		}
	...
	public List<Date> findAllComunicateDates(JyCompanionMessage message) {
		String sql = "select senderTime senderTime from JyCompanionMessage where " + "(userIdSender=:userIdSender and userIdReceiver=:userIdReceiver ) "
	 ...
	 */
	@Override
	public List<JyCompanionMessage> getPreMessages(Integer currentUserId, Integer companionUserId, Date date, Integer pageSize) {
		String sql = "select * from JyCompanionMessage where senderTime<:date and ((userIdSender=:currentUserId and userIdReceiver=:companionUserId) "
	 ...
	 */
	@Override
	public List<JyCompanionMessage> getNextMessages(Integer currentUserId, Integer companionUserId, Date date, Integer pageSize) {
		String sql = "select * from JyCompanionMessage where senderTime>:date and ((userIdSender=:currentUserId and userIdReceiver=:companionUserId) "
	...
	public Date getFirstMessageDate(Integer currentUserId, Integer userId) {
		String sql = "select senderTime senderTime from JyCompanionMessage  where " + "(userIdSender=:currentUserId and userIdReceiver=:companionUserId) "
	...
	public List<JyCompanion> findMsgUser(Integer id, String username) {
		...
		if (StringUtils.isNotEmpty(username)) {
			hql = "SELECT DISTINCT userIdSender as userIdCompanion FROM JyCompanionMessage where userIdReceiver=? and userNameSender like '%"+username+"%' ORDER BY senderTime DESC";
